                ...
                <div class="quick_view_slide">
                    @foreach ($thumbnails as $thumb)
                        <div class="single_view_slide">
                            <a href="{{ asset('uploads/products/thumbnails') }}/{{ $thumb->thumbnail }}" data-lightbox="roadtrip" class="d-block mb-4 w-100 img-fluid">
                                <img src="{{ asset('uploads/products/thumbnails') }}/{{ $thumb->thumbnail }}" class="img-fluid rounded w-100 img-fluid" alt="" />
                            </a>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="prd_details pl-3">
                    ...
                    <div class="prt_02 mb-3">
                        ...
                        @php
                            $ratings = 0;
                            if ($total_reviews != 0){
                                $ratings = $total_stars/$total_reviews;
                            }
                        @endphp
                        <div class="text-left">
                            <div class="star-rating align-items-center d-flex justify-content-left mb-1 p-0">
                                ...
                                @for ($i=1; $i <= $ratings; $i++)
                                    <label class="star" style="color: rgb(274 168 9)">★</label>
                                @endfor
                    ...
                    <div class="prt_04 mb-4">
                        ...
                        <div class="text-left pb-0 pt-2" id="size_id">
                            ...
                            @else
                                ...
                                @if ($sizes->rel_to_size->id == 1)
                                    <div class="form-check size-option form-option form-check-inline mb-2">
                                        <input class="form-check-input" type="radio" checked value="{{ $sizes->rel_to_size->id }}" name="size_id" id="{{ $sizes->id }}">
                                        <label class="form-option-label" for="{{ $sizes->id }}">{{ $sizes->rel_to_size->size }}</label>
                                    </div>
                                @else
                                    <div class="form-check size-option form-option form-check-inline mb-2">
                                        <input class="form-check-input" type="radio" value="{{ $sizes->rel_to_size->id }}" name="size_id" id="{{ $sizes->id }}">
                                        <label class="form-option-label" for="{{ $sizes->id }}">{{ $sizes->rel_to_size->size }}</label>
                                    </div>
                                @endif
                                @endforeach
                    ...
                    <div class="prt_05 mb-4">
                        ...
                        @if (session('stock'))
                            <span class="text-danger">{{ session('stock') }}</span>
                        @endif
                    </div>
    ...
    <div class="container">
        <div class="row align-items-center justify-content-center">
                                ...
                                <div class="single_rev d-flex align-items-start br-bottom py-3">
                                    ...
                                    <div class="single_rev_caption d-flex align-items-start pl-3">
                                        <div class="single_capt_left">
                                            ...
                                            <div class="star-rating align-items-center d-flex justify-content-left mb-1 p-0 mt-1">
            ...
            {{-- IF CONDITION START --}}
                        ...
                        @else
                            <div class="alert alert-warning"> <strong>You already leave a review</strong> </div>
                        @endif

                        @else
                            <div class="alert alert-warning"> <strong>You didn't purchase any product yet</strong> </div>
                        @endif
            {{-- IF CONDITON END --}}
                        @else
                            <div class="alert alert-warning"> <strong>Please login to leave a review</strong> <a href="{{ route('customer.reglogin') }}" style="float: right; font-weight: bold; color: green;">Login Here</a> </div>
                        @endauth
            {{-- AUTH CONDITION END --}}
                    </div>
                </div>
                            ...
                            <div class="product_grid card b-0 mb-0">
                                ...
                                <div class="card-body p-0">
                                    <div class="shop_thumb position-relative">
                                        <a class="card-img-top d-block overflow-hidden w-100 img-fluid" href="{{ route('product', $products->slug) }}"><img class="card-img-top" src="{{ asset('uploads/products/preview/'.$products->preview) }}" alt="..."></a>
                                    </div>
                                </div>
                                <div class="card-footer b-0 p-3 pb-0 d-flex align-items-start justify-content-center">
                                    <div class="text-left">
                                        <div class="text-center">
                                            <h5 class="fw-bolder fs-md mb-0 lh-1 mb-1"><a href="shop-single-v1.html">{{ $products->product_name }}</a></h5>
                                            <div class="elis_rty"><span class="ft-bold fs-md text-dark">BDT {{ $products->after_discount }}</span></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
@section('footer_script')
    ...
    @if (session('wishlist_removed'))
        <script>
            ...
            Toast.fire({
            icon: 'success',
            title: '{{ session('wishlist_removed') }}'
            })
        </script>
    @endif
@endsection
